{
        "translatorID": "afea7040-40d1-40cc-915a-7b4e308d81d6",
        "label": "BibTeX custom",
        "creator": "Simon Kornblith and Richard Karnesky, with modifications by Jonathan Whiteley",
        "target": "bib",
        "minVersion": "2.1.9",
        "maxVersion": "",
        "priority": 150,
 ...
 * It was modified to use a specified field in Zotero (the `extra` field by default) for a manual citation key, 
 * but also auto-generates one if no manual key is found (e.g. the field is empty).
 *
 * Modifications by Jonathan Whiteley (jaw)
 * jonathan[dot]whiteley[at]mail[dot]mcgill[dot]ca
 */
/* TODO --jaw
 * [done] extract citation key from BibTeX file on import.
...
/*jaw: I want to put the citation key into item.extra.
 * Comment out the following else if clause to put `note` into a Note in Zotero, rather than the `extra` field
*/ //jaw
	} else if(field == "note") {
		item.extra += "\n"+value;
	...
	} else if (field == "comment" || field == "annote" || field == "review" || field == "note") {   //jaw
		//jaw: added field=="note".  I want to put the citation keys into item.extra, instead of `note`?
...
function beginRecord(type, closeChar) {
	...
	var field = "";
	var citekey = "";  //jaw: variable to hold the BibTeX Citation Key
	...
	while(dontRead || (read = Zotero.read(1))) {
		...
		// read whitespace
			...
			//jaw: capture citation key when first comma is encountered :)
            if(citekey == "") citekey = field; //jaw
			field = "";
		} else if(read == closeChar) {
			if(item) {
				if(item.extra) item.extra = item.extra.substr(1); // chop \n
				if(item[citeKeyField]) {
                  item[citeKeyField] = citekey+"\n"+item[citeKeyField]; //jaw: prepend citation key & newline before existing content
                } else {
                  item[citeKeyField] = citekey;  //jaw: put citation key into appropriate field
                }
				item.complete();
			}
			return;
...
function buildCiteKey (item,citekeys) {
    var itemKey = item[citeKeyField];  //jaw get contents of the field where the cite key is (see preference var below `citeKeyFormat` above)
    var keyEnd  = itemKey.indexOf("\n");  //jaw find first newLine character
    if (keyEnd > 0) { 
      item[citeKeyField] = itemKey.substr(keyEnd+1);   //jaw: strip citation key & \n from source field
      itemKey = itemKey.substr(0, keyEnd);   //jaw: keep only the first line, in case the field contains other data
    }
    if (itemKey == "") {    //jaw - try the item's field contents; only auto-generate if blank
        var basekey = "";
    	var counter = 0;
        ...
        citekey = itemKey;  //jaw if the item's key field is not blank, use that without cleaning it (you want the power, you get the responsibility).
    }  //jaw
...
/*jaw: I'm using the `extra` field for manual citation keys: no need to write this field to `note`
        // On the other hand, putting it here makes it easier to import (using the default BibTeX Translator),
        // and keeps the data available for other applications / scripts.
        //   It would be 'cleaner' to avoid doing this, but until I can figure out how to 
        //   extract the citation key from the raw BibTeX on import, it's probably safest.
        //jaw any other 'extra' data that can go in a `note`?
*/ //jaw		
        if(item.extra) {
...
//jaw: Export methods & properties to other translators
/*jaw
 * I don't really understand what role this plays, 
 * as it does not seem to be necessary to access functions here when imported into another translator, 
...
var exports = {
	...
	"citeKeyField":citeKeyField,
	"citeKeyFormat":citeKeyFormat,
	"citeKeyPrefix":citeKeyPrefix,
	"citeKeySuffix":citeKeySuffix
};
